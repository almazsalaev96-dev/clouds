...
| **Gold is the action** ✅ | The signal colour is gold, not cyan, on the evidence in RESEARCH §4i: on black `#d9b45a` reads at 10.6 (the cyan's 11.4), the button `#c29e3f` carries its ink at 7.2, the reading ink `#c09e4a` is solved to the same 7.0, and warm-on-black is the reference read. It is the *action* colour only — send disc, the one CTA, the live ring and dot, the caret's edge, the search mark, the wordmark's letter. Blue stays structure; the marker stays the highlighter (ΔE 10.5). In light, gold is never text (1.9:1 at `#d4af5a`): the button is an old gold (`#ac8830`, the lightest that holds the app's own 3:1 edge rule on the page, 5.5 with its ink), the highlight chip and the mark take the bright gold with an ochre edge, letterforms take the ochre ink (`#8c6b06` 4.6, `#6e4e00` 7.0). The warning colour moved a step toward red (`#f39a6e` dark, `#9c3b00` light) to stay ≥ 8.5 ΔE from the gold. Gate: `contrast`, `theme-parity`, `type-scale`, `test-tokens`, `depth`. | — |
