...
| **Word and PowerPoint out** ✅ | Notebook page header and the answer menu: *Save as Word* (`markdownToDocx`: headings, paragraphs, bullets, numbered lines, quotes, code, table rows, inline bold/italic/code). Creations: *Print or save as PDF* on any built page (`printHtml`), and *Download as PowerPoint* on a deck (`deckToPptx`: heading, bullets, paragraphs, big numbers, speaker notes per slide, 16:9). Both libraries load only when pressed. Gate: `e2e-office`. | Excel workbooks; pictures drawn in CSS are not carried into the .pptx. |
...
| **Save as PDF** ✅ | A Notebook page (header), a revision pack (*Save the pack as PDF*) and any answer (its menu) open typeset in a window of their own — title, headings, emphasis, lists, quotes, tables, code — and the browser's print dialog offers Save as PDF or the share sheet. No PDF library: the browser sets every language and font. Gate: `e2e-pdf`. | — |
