...
| **Navy, blue, violet, neon** ✅ | The palette Almaz specified, applied as given. Dark: page `#080B12`, panels and sidebar `#0E1420`, cards `#121A27`, raised `#182231`, borders `#253244`, text `#F5F7FA` / `#9AA6B5`; blue `#4F8CFF` is the AI and the action (send disc, CTA, links, focus) with near-black ink on it (6.1 — white would be 3.2); violet `#8B5CF6` rides beside it in every gradient, glow and the model icon, and never carries a sentence (4.1 on a card) — where it must be a letter it is `#b489ff` (7.0); neon `#C7F000` is the small special accent — the live dot and the highlight chip only; emerald `#22C55E`, amber `#F59E0B`, red `#EF4444` for status (red fills are `#dc2626` so white reads at 4.8). Derived, not given: text steps three and four (`#8893a2`, `#828d9c`, ≥4.7 on raised), a strong border that holds 3:1 (`#5f7089`). Light takes the same family, each darkened only as far as a light page needs; its text ladder was re-stepped (`#0b0e14`, `#4a5261`, …) so both themes still climb by the same rungs. Replaces the gold round. Gate: `contrast`, `theme-parity`, `type-scale`, `depth`, `audit`, `test-tokens` — all first time. | — |
