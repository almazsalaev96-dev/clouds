...
| **Library** ✅ | The room formerly called Artifacts, and Code before that. "Code" sent everyone who was not a programmer straight past a room holding decks, timetables and trackers; "Artifacts" is a word this industry uses and nobody else does. "Library" is what a person calls the place their own things are kept. Same room, same `Section` id (`code`), so nothing stored is migrated. | Absorb pages, decks and revision packs so there is one place for everything made, not two. |
| **Plugins (connectors)** ✅ | Outside services as tools, over the **Model Context Protocol** rather than one hand-written integration per company — it is what the connectors in the tools this is compared against run on underneath, it fits bring-your-own-key exactly (a server address and a token, the way a key is brought), and this app holds no client secret it could leak. `lib/mcp.ts` speaks JSON-RPC (`initialize`, `tools/list`, `tools/call`) through `app/api/mcp`, a pass-through that holds and remembers nothing: a browser cannot reach most MCP endpoints across origins, and the token should not ride in a preflight. Every tool is renamed `service__tool` so nothing can shadow a room tool, and the tools fold into the existing `actionSpecs`/`runAction` machinery — rooms listed first, because the ordinary way to save a card is the card room and not somebody's server. A connector arrives **off**: its whole tool list is shown, by name and description, and nothing is offered to a model until it is switched on. No undo, ever — a card this app saved can be unsaved; an email somebody else's server sent is gone. The proxy refuses anything that is not https and any private or loopback host, `169.254.169.254` included, because an open proxy reaches whatever the server can reach. A row in the sidebar under the rooms, not a seventh room: it is a setup task, not a destination, and one of the two strong seats should not go to something you touch once. Gate: `test-plugins`, `e2e-plugins`. | OAuth, so a service can be authorised rather than pasted; a catalogue of known servers. |
